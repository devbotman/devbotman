## Hey, I'm Devon O'Rourke 👋

**Distinguished Technical Program Manager** · Boston, MA · Open to Staff+ TPM & Associate Director roles

I spend my days delivering complex technical programs — 5G infrastructure, cloud platforms, hardware/software integrated systems. I spend my evenings building tools to make that work better.

---

### 🛠️ Projects

| Project | What it does | Stack |
|---------|-------------|-------|
| [🎯 TPM Command Center](https://github.com/devbotman/tpm-command-center) | Self-hosted job search automation — scrapes 250+ ATS boards, scores jobs with a local LLM, real-time Flask dashboard | Python · Playwright · Ollama · Flask · SQLite |
| [🏠 MA Housing Tracker](https://github.com/devbotman/ma-housing-tracker) | Housing market analytics — MA prices by town/county, property taxes, mortgage rates charted over 60 days | Python · FRED API · Zillow Data · Chart.js |

---

### 📍 Background

- 🏢 **Distinguished TPM @ Verizon** — 10 years
- 📡 **5G NR delivery** — Gillette Stadium, Xfinity Center, $8.38M broadband program
- ☁️ **Cloud infrastructure** — GCP + AWS platform programs, 40% deployment time reduction
- ⚙️ **Process re-engineering** — 90,000 engineering hours recovered
- 💰 **$200M** vertical product opportunity identified & piloted

---

### 🔧 Stack

```
Program: Agile · SAFe · JIRA · Rally · Confluence · OKRs · Risk Mgmt
Cloud:   GCP · AWS · BigQuery · SQL · CI/CD
Code:    Python · Playwright · Flask · SQLite · Pandas
Domain:  5G NR · Telecom · Network Engineering · Hardware/Software SDLC
```

---

### 📫 Reach me

[![LinkedIn](https://img.shields.io/badge/LinkedIn-devon--orourke--pat-0077B5?style=flat&logo=linkedin)](https://www.linkedin.com/in/devon-orourke-pat/)
[![Portfolio](https://img.shields.io/badge/Portfolio-devbotman.github.io-58a6ff?style=flat&logo=github)](https://devbotman.github.io)
[![Email](https://img.shields.io/badge/Email-orourke.landry@gmail.com-D14836?style=flat&logo=gmail)](mailto:orourke.landry@gmail.com)
